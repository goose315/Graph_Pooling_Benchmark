...
class GNNPred(torch.nn.Module):
    ...
    def GNN(self, data):
        ...
        for i in range(self.num_layers):
            ...
            embds.append(embd)   
            #print(f"Layer {i}: embd size: {embd.size()}") 
        
        min_size = min([embd.size(0) for embd in embds])
        embds = [embd[:min_size] for embd in embds]
        try:
...
class TopK(GIN):
    def __init__(self, *args, ratio=0.5, **kwargs):
        # Store the ratio before calling the parent class constructor
        self.ratio = ratio
        # Now call the parent constructor
        super(TopK, self).__init__(*args, **kwargs)
    
    @property
    def if_pool(self):
        return True
    
    def init_pool(self, in_channels: int, **kwargs):
        # Remove 'ratio' from kwargs if it exists to avoid conflict
        kwargs.pop('ratio', None)
        # Use the ratio stored in self.ratio when initializing TopKPooling
        return TopKPooling(in_channels=in_channels, ratio=self.ratio, **kwargs)
...
class SAG(GIN):
    def __init__(self, *args, ratio=0.5, **kwargs):
        ...
        self.ratio = ratio
        # Now call the parent constructor
        super(SAG, self).__init__(*args, **kwargs)
    ...
    def init_pool(self, in_channels: int, **kwargs):
        # Remove 'ratio' from kwargs if it exists to avoid conflict
        kwargs.pop('ratio', None)
        # Use the ratio stored in self.ratio when initializing SAGPooling
        return SAGPooling(in_channels=in_channels, ratio=self.ratio, **kwargs)
...
class ASAP(GIN):
    def __init__(self, *args, ratio=0.5, **kwargs):
        ...
        self.ratio = ratio
        # Now call the parent constructor
        super(ASAP, self).__init__(*args, **kwargs)
    ...
    def init_pool(self, in_channels: int, **kwargs):
        # Remove 'ratio' from kwargs if it exists to avoid conflict
        kwargs.pop('ratio', None)
        # Use the ratio stored in self.ratio when initializing ASAPooling
        return ASAPooling(in_channels=in_channels, ratio=self.ratio, **kwargs)
...
class PAN(GIN):
    def __init__(self, *args, ratio=0.5, **kwargs):
        ...
        self.ratio = ratio
        # Now call the parent constructor
        super(PAN, self).__init__(*args, **kwargs)
    ...
    def init_pool(self, in_channels: int, **kwargs):
        # Remove 'ratio' from kwargs if it exists to avoid conflict
        kwargs.pop('ratio', None)
        # Use the ratio stored in self.ratio when initializing PANPooling
        return PANPooling(in_channels=in_channels, ratio=self.ratio, **kwargs)
...
class CGIPool(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr=None, batch=None):
        ...
        perm_negative = perm_negative[perm_negative < x_transform.size(0)]
        perm_positive = perm_positive[perm_positive < x_transform.size(0)]
        if perm_positive.max() >= x_transform.size(0):
            raise ValueError(f"Invalid index found in perm_positive: {perm_negative.max()} is out of bounds.")
        if perm_negative.max() >= x_transform.size(0):
            raise ValueError(f"Invalid index found in perm_negative: {perm_negative.max()} is out of bounds.")
        ...
        min_size = min(x_pp_readout.size(0), x_readout.size(0))
        if x_pp_readout.size(0) != min_size:
            x_pp_readout = x_pp_readout[:min_size]
        if x_readout.size(0) != min_size:
            x_readout = x_readout[:min_size]

        positive_pair = torch.cat([x_pp_readout, x_readout], dim=1)

        min_size = min(x_np_readout.size(0), x_readout.size(0))
        if x_np_readout.size(0) != min_size:
            x_np_readout = x_np_readout[:min_size]
        if x_readout.size(0) != min_size:
            x_readout = x_readout[:min_size]
        ...
        perm = topk(score, self.ratio, batch)
        perm = perm[perm < x_transform.size(0)]
        if perm.size(0) == 0:
            raise ValueError("No valid indices found in perm.")
...
class CGI(GIN):
    def __init__(self, *args, ratio=0.5, **kwargs):
        ...
        self.ratio = ratio
        # Now call the parent constructor
        super(CGI, self).__init__(*args, **kwargs)
    ...
    def init_pool(self, in_channels: int, **kwargs):
        # Remove 'ratio' from kwargs if it exists to avoid conflict
        kwargs.pop('ratio', None)
        # Use the ratio stored in self.ratio when initializing CGIPool
        return CGIPool(in_channels=in_channels, ratio=self.ratio, **kwargs)
...
class KMIS(GIN):
    ...
    def init_pool(self, in_channels: int, ratio: float,
                  **kwargs):
        return KMISPooling(64, k=1, aggr_x='sum')
...
class GSA(GIN):
    def __init__(self, *args, ratio=0.5, alpha=0.6, cus_drop_ratio=0, **kwargs):
        # Store the parameters before calling the parent class constructor
        self.ratio = ratio
        self.alpha = alpha
        self.cus_drop_ratio = cus_drop_ratio
        # Now call the parent constructor
        super(GSA, self).__init__(*args, **kwargs)
    ...
    def init_pool(self, in_channels: int, **kwargs):
        # Use the stored parameters when initializing GSAPool
        return GSAPool(in_channels, pooling_ratio=self.ratio, alpha=self.alpha, cus_drop_ratio=self.cus_drop_ratio)
    
    def step_pool(self, pool, x, edge_index, batch):
        # Perform the pooling step and return the results
...
class MinCut(DensePool):
    ...
    def densepool(self, x, adj, s, mask=None, lam = 0.5,
                  **kwargs):
        x, adj, ml, ol = dense_mincut_pool(x, adj, s, temp=3)
        #print(x)
        #l = lam * ml + (1-lam) * ol
        return x, adj
...
class DMoN(DensePool):
    def init_conv_pool(self, in_channels: int, out_channels: int,
                       **kwargs):
        self.pool1 = DMoNPooling([64, 128], 12)
        mlp = Sequential(Linear(in_channels, out_channels),
                         ReLU(),
                         Linear(out_channels, out_channels),
                         ReLU())
...
class Hosc(DensePool):
    ...
    def densepool(self, x, adj, s, mask=None, lam = 0.5,
                  **kwargs):
        x, adj, ml, ol = dense_hoscpool(x, adj, s, mu=0.3, alpha=0.5, new_ortho=False, mask=mask)
        l = lam * ml + (1-lam) * ol
        return x, adj
