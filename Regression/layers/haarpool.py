...
def HaarPoolFunc(x, edge_index, U, edge_index_list, num_nodes_tree, batch, batch_size, L):
    """
    HaarPooling
    INPUT:
        x                 - input data for batch, size = [Num_nodes Num_features]
    ...
    for i in range(batch_size):
        ...
        edge_index_i = edge_index_list[i][L-1].to(device) # extract the edge_index of Lth layer of tree for ith sample of batch
        s_num_node_i = 0
        for j in range(i):
        ...
        if i==0:
            x_1 = xi
            edge_index_1 = edge_index_i.to(device)
            batch_1 = batch_i
        if i>0:
...
class HaarPooling(torch.nn.Module):
    def __init__(self, L=1):
        super().__init__()
        self.L = L
    def forward(self, x, edge_index, U, edge_index_list, num_nodes_tree, batch, batch_size):
        return HaarPoolFunc(x, edge_index, U, edge_index_list, num_nodes_tree, batch, batch_size, self.L)
