...
def load_tree(dataname, tree_depth=2):
    if not os.path.exists('data'):
        os.makedirs('data')
    dataset = GitHub(root=os.path.join(PWD, 'data')) # This is for GitHub data
    #dataset = WebKB(root=os.path.join(PWD, 'data'), name=dataname) This is for Cornell, wiscon, Texas data
    #dataset = Planetoid(root=os.path.join(PWD, 'data'), name=dataname) This is for Pubmed, Cora, Citeseer
    data = dataset[0]
